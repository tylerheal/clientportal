...
{
    $now = (new \DateTimeImmutable())->format(\DateTimeInterface::ATOM);
    $baseDir = __DIR__ . '/templates/email/defaults';
    $templates = [
        [
    ...
    $update = $pdo->prepare('UPDATE email_templates SET subject = :subject, body = :body, updated_at = :updated WHERE slug = :slug');

    foreach ($templates as $template) {
        $file = $baseDir . '/' . $template['file'];
        if (!is_file($file)) {
            continue;
        }
        ...
        $select->execute(['slug' => $template['slug']]);
        ...
        $existingSubject = (string) ($existing['subject'] ?? '');
        $needsUpdate = stripos($existingBody, '<html') === false
            || strpos($existingSubject, '{{invoice}}') !== false
            || strpos($existingBody, '{{invoice}}') !== false;
        if ($template['slug'] === 'invoice_document') {
            $needsUpdate = $needsUpdate || stripos($existingBody, '<!doctype html>') === false;
        }
