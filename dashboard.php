    ...
    try {
        switch ($action) {
            ...
            case 'update_settings':
                ...
                $brandAddress = trim($_POST['brand_address'] ?? '');
                $brandSupportUrl = trim($_POST['brand_support_url'] ?? '');
                ...
                $settings = [
                    ...
                    'brand_url' => $brandUrl,
                    'brand_support_url' => $brandSupportUrl !== '' ? $brandSupportUrl : $brandUrl,
            ...
            case 'send_test_email':
                ...
                if ($templateSlug !== '') {
                    ...
                    $clientContext = email_client_context($sampleUser);
                    $itemsHtml = email_order_items_html([
                        ['name' => 'Sample Service', 'amount' => format_currency(149.00)],
                        ['name' => 'Onboarding Package', 'amount' => format_currency(50.00)],
                    ]);
                    ...
                    $testContext = [
                        ...
                        'order' => [
                            'id' => email_safe('1234'),
                            'number' => email_safe('#1234'),
                            'date' => email_safe($now->format('j M Y')),
                            'url' => email_safe(absolute_url('dashboard/orders/1234')),
                            'subtotal' => email_safe(format_currency(149.00)),
                            'vat' => email_safe(format_currency(30.00)),
                            'total' => email_safe(format_currency(179.00)),
                            'payment_method' => email_safe(email_payment_method_label('manual')),
                            'items_html' => $itemsHtml,
                        ],
                        'ticket' => [
                            'id' => email_safe('58'),
                            'subject' => email_safe('Sample Ticket Subject'),
                            'status' => email_safe('Open'),
                            'priority' => email_safe('Normal'),
                            'url' => email_safe(absolute_url('admin/tickets/58')),
                        ],
                        'message' => [
                            'snippet' => email_safe('This is placeholder content generated by the email test tool.'),
                        ],
                        'invoice' => [
                        ...
                        'service' => email_safe('Sample Service'),
                        'name' => $clientContext['full_name'],
                    ];

                    $sent = send_templated_email($pdo, $templateSlug, $testContext, $recipient, $genericSubject, $genericBody);
                } else {
            ...
            case 'create_order':
                ...
                $now = $placedAt->format(\DateTimeInterface::ATOM);
                ...
                $orderDate = $placedAt->format('j M Y');
                ...
                if ($orderId > 0) {
                    ...
                    if ($paymentStatusChangedToPaid) {
                        ...
                        if ($row = $orderUser->fetch()) {
                            ...
                            $clientInfo = email_client_context(['name' => $row['name'], 'email' => $row['email']]);
            ...
            case 'create_ticket':
                ...
                $clientContext = email_client_context($user);
                $ticketUrl = absolute_url('dashboard/tickets/' . $ticketId);
                $ticketContext = [
                    'client' => $clientContext,
                    'ticket' => [
                        'id' => email_safe((string) $ticketId),
                        'subject' => email_safe($subject),
                        'status' => email_safe('Open'),
                        'priority' => email_safe('Normal'),
                        'url' => email_safe($ticketUrl),
                    ],
                    'message' => [
                        'snippet' => email_safe(email_snippet($message)),
                    ],
                    'name' => $clientContext['full_name'],
                ];
                $ticketBody = sprintf(
                    "Hi %s,\n\nWe've opened ticket #%s about '%s'. We'll be in touch soon.",
                    $user['name'],
                    $ticketId,
                    $subject
                );
                send_templated_email($pdo, 'client_ticket_opened', $ticketContext, $user['email'], 'Ticket created', $ticketBody);
                ...
                $adminTicketContext = [
                    ...
                    'ticket' => [
                        ...
                        'status' => email_safe('Open'),
                        'priority' => email_safe('Normal'),
                        'url' => email_safe(absolute_url('admin/tickets/' . $ticketId)),
                    ],
                    'message' => [
                        'snippet' => email_safe(email_snippet($message)),
                    ],
                ];
                notify_admins($pdo, 'New ticket #' . $ticketId . ' from ' . $user['name'], url_for('admin/tickets/' . $ticketId), 'admin_new_ticket', $adminTicketContext);
            ...
            case 'reply_ticket_admin':
                ...
                if ($row = $ticket->fetch()) {
                    ...
                    $ticketContext = [
                        ...
                        'ticket' => [
                            'id' => email_safe((string) $ticketId),
                            'subject' => email_safe($row['subject']),
                            'status' => email_safe(ucfirst($status)),
                            'priority' => email_safe('Normal'),
                            'url' => email_safe(absolute_url('dashboard/tickets/' . $ticketId)),
                        ],
                        'message' => [
                            'snippet' => email_safe(email_snippet($message)),
                        ],
                    ...
                    send_templated_email($pdo, 'ticket_reply', $ticketContext, $row['email'], 'Ticket update', $ticketBody);
            ...
            case 'reply_ticket_client':
                ...
                $clientContext = email_client_context($user);
                $ticketSubjectStmt = $pdo->prepare('SELECT subject FROM tickets WHERE id = :id LIMIT 1');
                $ticketSubjectStmt->execute(['id' => $ticketId]);
                $clientTicketSubject = trim((string) $ticketSubjectStmt->fetchColumn());
                $adminReplyContext = [
                    'client' => $clientContext,
                    'ticket' => [
                        'id' => email_safe((string) $ticketId),
                        'subject' => email_safe($clientTicketSubject !== '' ? $clientTicketSubject : 'Ticket #' . $ticketId),
                        'status' => email_safe('Open'),
                        'priority' => email_safe('Normal'),
                        'url' => email_safe(absolute_url('admin/tickets/' . $ticketId)),
                    ],
                ...
                notify_admins($pdo, 'Client replied to ticket #' . $ticketId, url_for('admin/tickets/' . $ticketId), 'admin_ticket_reply', $adminReplyContext);
